...
public class GeneratorHelper {
    ...
    public void generateEntityFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        columns.forEach(c -> {
            c.setField(MyUtil.underscoreToCamel(StringUtils.lowerCase(c.getName())));
    ...
    public void generateMapperXmlFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        columns.forEach(c -> c.setField(MyUtil.underscoreToCamel(StringUtils.lowerCase(c.getName()))));
        configure.setColumns(columns);
    ...
    private Template getTemplate(String templateName) throws Exception {
        ...
        String generatorBasePath="/generator/templates/";
        String templatePath = GeneratorHelper.class.getResource(generatorBasePath).getPath();
        File file = new File(templatePath);
        if (!file.exists()) {
            templatePath = System.getProperties().getProperty("java.io.tmpdir");
            file = new File(templatePath + File.separator + templateName);
            InputStream resourceAsStream = GeneratorHelper.class.getResourceAsStream(generatorBasePath + templateName);
