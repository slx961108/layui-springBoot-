    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading website-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal website-alert',
            ...
            self.tab = {
                ...
                minLeft: null,
                maxLeft: null,
                wrap: '.website-tabs-wrap',
                menu: '.website-tabs-menu',
                next: '.website-tabs-next',
                prev: '.website-tabs-prev',
                step: 200,
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .website-tabs-btn';

                    layui.dropdown.render({
                        elem: '.website-tabs-down',
                        click: function (name) {
                            if (name === 'all') {
                                tab.delAll();
                            }
                            ...
                            if (name === 'current') {
                                tab.del(layui.website.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('website-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.website-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .website-tabs-btn';
                ...
                delOther: function () {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .website-tabs-btn';
                    $(menuBtnClas + '.website-tabs-active')
                        .siblings()
                        .each(function () {
                            var url = $(this).attr('lay-url');
                            tab.del(url)
                ...
                del: function (url, backgroundDel) {
                    if (url === layui.website.defaultView.href) {
                        layui.website.alert.warn(layui.website.defaultView.title + '无法删除！')
                        return;
                    }
                    var tab = this;
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .website-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('website-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.website.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .website-tabs-btn')
                },
                change: function (route, callback, options) {
                    ...
                    var activeCls = 'website-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.website.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='website-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            layui.website.render(tab.tabMenuTplId);
                    ...
                    layui.website.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('website-tabs-body')
                    }
                    layui.website.appBody = self.containerBody;
